import React, { useState } from 'react';
import walletBg from "../assets/walletbg.jpg"
import sprinkles from "../assets/sp.svg"
import { Link } from 'react-router-dom';

const Wallet = () => {

    const [see, setSee] = useState(false)

    const handleSee = () => setSee(!see)
    

    return (
        <div>
            <div className="wallet pt-4 pb-6 md:w-1/2">
                
            
                 <h1 className="pl-2 pb-2 text-xl font-semibold text-slate-800">Auction Ads Account</h1>
                <div className="relative border bg-black rounded-md h-44 overflow-hidden">
                 
                 <img src={sprinkles} className="top-0" alt="" />
                 
                 <div className="absolute top-4 left-3 text-white font-semibold">
                   <div>Ads Wallet balance</div>
                   <div>Naira</div>
                   <div className="text-2xl flex gap-4 items-center">
                    <div className="">{see ? "10,000" : "X X X X"} </div>
                    <div className='cursor-pointer' onClick={handleSee}>
                    {!see && (
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 text-">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M3.98 8.223A10.477 10.477 0 001.934 12C3.226 16.338 7.244 19.5 12 19.5c.993 0 1.953-.138 2.863-.395M6.228 6.228A10.45 10.45 0 0112 4.5c4.756 0 8.773 3.162 10.065 7.498a10.523 10.523 0 01-4.293 5.774M6.228 6.228L3 3m3.228 3.228l3.65 3.65m7.894 7.894L21 21m-3.228-3.228l-3.65-3.65m0 0a3 3 0 10-4.243-4.243m4.242 4.242L9.88 9.88" />
                        </svg>
                    )}
                    {see && (
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" />
                        <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                      </svg>
                      
                    )}
                   
                    </div>
                    </div>
                   <div className="mt-4">
                   <Link to="/login"><button className="py-2 px-6 rounded-lg bg-primary text-white">Go to Wallet</button></Link>
                <Link to="/register"> <button className="py-2 px-6 rounded-lg border border-primary text-slate-300">Fund wallet</button></Link>

                   </div>
                 </div>
                 
                 </div>

             </div>
        </div>
    );
}

export default Wallet;
